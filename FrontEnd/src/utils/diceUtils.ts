...
const rollTraitAction = (trait: TraitAction): string => {
  return JSON.stringify({
    ...
    damage: trait.damage.reduce((acc, cur) => [...acc, {
      roll: parse(cur.roll),
      damageType: cur.damageType,
    }], [] as RolledDamageEntry[]),
    other: trait.other.reduce((acc, cur) => [...acc, parse(cur)], [] as ParseResultType[]),
  })
}
...
export const sendSimpleDiceRoll = async (diceExpression: string, description: string = '') => {
